    ...
    def clean(self):
        ...
        if user:
            if user.is_active:
                ...
                raise forms.ValidationError(_("This account is currently inactive. To activate, please contact us at info@architecturenow.com.au."))
        else:
            if EMAIL_AUTHENTICATION:
    ...
    def clean_email(self):
        ...
        if UNIQUE_EMAIL or EMAIL_AUTHENTICATION:
            try:
                ...
                return value
            raise forms.ValidationError(_("This address is already in use. <a href="{% url acct_passwd_reset %}"><em>{% trans "Forgot password?" %}</em></a>"))
        return value
